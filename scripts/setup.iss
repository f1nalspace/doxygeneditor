...
begin
  ...
  netVersion := ExpandConstant('{#RequiredNETVersion}');
  Result := IsDotNetDetected(netVersion, 0);
  if not Result then
  begin
    MsgBox('.NET Framework '+netVersion+' or higher is required to install "'+appName+'"!', mbCriticalError, MB_OK);
  end
end;