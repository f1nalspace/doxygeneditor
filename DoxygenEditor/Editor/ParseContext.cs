...
namespace TSP.DoxygenEditor.Editor
{
    class ParseContext : IParseControl, IParseInfo, IDisposable
    {
        ...
        private TokenizeResult TokenizeCpp(string text, string filePath, TextPosition pos, int length, LanguageKind lang)
        {
            TokenizeResult result = new TokenizeResult();
            Stopwatch timer = new Stopwatch();
            timer.Restart();
            List<CppToken> cppTokens = new List<CppToken>();
            using (CppLexer cppLexer = new CppLexer(text, filePath, pos, length, lang))
            {
            ...
            foreach (CppToken token in cppTokens)
            {
                if ((lang == LanguageKind.Cpp) && (token.Kind == CppTokenKind.MultiLineCommentDoc || token.Kind == CppTokenKind.SingleLineCommentDoc))
                {
                    ...
                    using (TokenizeResult doxyRes = TokenizeDoxy(text, filePath, token.Position, token.Length))
                    {
        ...
        private TokenizeResult TokenizeHtml(string text, string filePath, TextPosition pos, int length)
        {
            TokenizeResult result = new TokenizeResult();
            Stopwatch timer = Stopwatch.StartNew();
            using (HtmlLexer htmlLexer = new HtmlLexer(text, filePath, pos, length))
            {
        ...
        private TokenizeResult TokenizeDoxy(string text, string filePath, TextPosition pos, int length)
        {
            TokenizeResult result = new TokenizeResult();
            ...
            using (DoxygenBlockLexer doxyLexer = new DoxygenBlockLexer(text, filePath, pos, length))
            {
            ...
            while (curLink != null)
            {
                        ...
                        if (endRule.StartCommandNames.Contains(topStartState.CommandName))
                        {
                            ...
                            if ("code".Equals(topStartState.CommandName))
                            {
                                ...
                                if ("{.c}".Equals(codeType, StringComparison.InvariantCultureIgnoreCase) || "{.cpp}".Equals(codeType, StringComparison.InvariantCultureIgnoreCase))
                                {
                                    ...
                                    using (TokenizeResult cppRes = TokenizeCpp(text, filePath, commandContentStart, commandContentLength, LanguageKind.DoxygenCode))
                                    {
                ...
                else if (doxyToken.Kind == DoxygenTokenKind.TextEnd)
                {
                    if (textStartTokens.Count > 0)
                    {
                        ...
                        using (TokenizeResult htmlRes = TokenizeHtml(text, filePath, textStartToken.Position, textContentLen))
                        {
        ...
        private void Tokenize(string text)
        {
            ...
            if (_id.FileType == EditorFileType.Cpp || _id.FileType == EditorFileType.DoxyDocs)
            {
                ...
                using (TokenizeResult cppRes = TokenizeCpp(text, _id.FilePath, new TextPosition(0), text.Length, LanguageKind.Cpp))
                {
            ...
            {
                using (DoxygenConfigLexer doxyConfigLexer = new DoxygenConfigLexer(text, _id.FilePath, new TextPosition(0), text.Length))
                {
                    Stopwatch timer = Stopwatch.StartNew();
